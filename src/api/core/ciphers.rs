...
use rocket::{http::ContentType, request::Form, Data, data::ToByteUnit, Route};
...
async fn post_attachment(
    ...
    data.open(size_limit.map(|l| (l + l/2).bytes()).unwrap_or(1u64.gigabytes())).stream_to_file(temp_file.clone()).await?;
    ...
    let res = tokio::task::spawn_blocking(move || {
        ...
        (cipher, conn, error) // Return them to avoid move errors
    })
    .await;
